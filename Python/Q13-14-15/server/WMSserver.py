...
import generateLayer

PORT_NUMBER = 4242

"""requete HTTP à mettre dans le browser :
   http://localhost:4242/wms?request=GetMap&layers=LayerName&height=800&width=1000&srs=3857&bbox=bboxvalue
"""
    ...
    def do_GET(self):
        if self.path.startswith("/wms"):
            ...
            # sinon on appelle le script de génération d'image , puis on envoie l'image au browser
                ...
                if request == "GetMap":
                    if srs == "EPSG:3857":
                        fileToServe = generateLayer.buildLayer(layers, int(height), int(width), 3857, bbox)
